				...
				<table id="datatable-fixed-header" class="table table-striped table-bordered" style="text-align: center;">
					<thead>
						<tr>
							...
							<th class="col-md-4" style="text-align: center;">身份证图片</th>
...
<%for(var i = 0; i < data.length; i++) {%>
	<tr>
		<td style="vertical-align: middle;"><a href="/admin/user_resource?uid=<%=data[i].user.uid%>"><%=data[i].user.uid%></a></td>
		<td><img src="<%=data[i].user.header_url%>" width="50px" height="50px"></td>
		<td style="vertical-align: middle;"><%=data[i].user.nickname%></td>
		<td style="vertical-align: middle;"><%if(data[i].user.sex==1){%>男<%}else{%>女<%}%></td>
		<td style="vertical-align: middle;text-align:left;">
			<%for(var j=0;j<data[i].imgs.length;j++){%>
 				<a class="example-image-link" data-lightbox="example-set"  href="<%=data[i].imgs[j].img_url%>" data-lightbox="lightbox[roadtrip]<%=data[i].user.uid%>">
					<img src="<%=data[i].imgs[j].img_url%>" width="50px" height="50px">
				</a>
			<%}%>
		</td>
		<td style="vertical-align: middle;text-align:left;">
			...
			<%if(data[i].status == 0) { %>
				<button type="submit" class="btn btn-success" value="<%=data[i].user.uid%>">通过</button>
				<button type="submit" class="btn btn-danger" value="<%=data[i].user.uid%>">不通过</button>
			<%}%>
			<%if(data[i].status == 1) { %>
				<button type="submit" class="btn btn-danger" value="<%=data[i].user.uid%>">不通过</button>
			<%}%>
